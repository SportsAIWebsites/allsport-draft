...
Snake draft, 8 teams (You, Buddy, + 6 bots), 12 rounds (96 picks). No
...
CBB_RAW: list[tuple[str, str, int, int, int]] = [
    # ---- Elite (89-96) ----
    ("AJ Dybantsa", "SF", 1, 96, 85),
    ("Cameron Boozer", "PF", 1, 94, 88),
    ("Darryn Peterson", "SG", 1, 93, 85),
    ("Braden Smith", "PG", 3, 90, 88),
    ("JT Toppin", "PF", 3, 90, 85),
    ("Nate Ament", "SF", 1, 89, 82),
    # ---- Quality (74-88) ----
    ("Caleb Wilson", "PF", 1, 88, 85),
    ("Labaron Philon", "PG", 2, 87, 85),
    ("Tahaad Pettiford", "PG", 2, 86, 82),
    ("RJ Luis Jr.", "SF", 3, 86, 82),
    ("Otega Oweh", "SG", 3, 85, 85),
    ("Boogie Fland", "PG", 2, 85, 78),
    ("Milos Uzan", "PG", 3, 84, 85),
    ("Flory Bidunga", "C", 2, 84, 85),
    ("Mark Sears", "PG", 4, 84, 85),
    ("Alex Condon", "C", 3, 83, 85),
    ("Isaiah Evans", "SG", 2, 83, 82),
    ("Egor Demin", "PG", 1, 83, 80),
    ("Walter Clayton Jr.", "PG", 4, 82, 78),
    ("Grant Nelson", "PF", 4, 82, 85),
    ("PJ Haggerty", "SG", 3, 82, 80),
    ("Ryan Kalkbrenner", "C", 4, 82, 88),
    ("Yaxel Lendeborg", "PF", 4, 81, 82),
    ("Koa Peat", "PF", 1, 80, 82),
    ("Kam Jones", "PG", 4, 80, 80),
    ("Jasper Johnson", "SG", 1, 80, 80),
    ("Chaz Lanier", "SG", 4, 80, 82),
    ("Alijah Martin", "SG", 4, 80, 82),
    ("Tyler Betsey", "SF", 2, 80, 80),
    ("Donovan Dent", "PG", 3, 80, 82),
    ("Vladislav Goldin", "C", 4, 78, 85),
    ("Cayden Boozer", "PG", 1, 78, 82),
    ("Jamir Watkins", "SF", 4, 78, 82),
    ("Great Osobor", "PF", 4, 77, 82),
    ("Rasheer Fleming", "PF", 3, 79, 82),
    ("Adou Thiero", "SF", 3, 79, 75),
    ("Meleek Thomas", "SG", 1, 79, 82),
    ("Kadary Richmond", "PG", 4, 78, 78),
    ("Sion James", "SF", 4, 74, 82),
    ("Tyrese Proctor", "PG", 4, 76, 82),
    ("Norchad Omier", "PF", 4, 76, 82),
    ("Zakai Zeigler", "PG", 4, 76, 70),
    ("Nate Bittle", "C", 4, 78, 78),
    ("Javon Small", "PG", 4, 76, 78),
    ("Jaland Lowe", "PG", 3, 76, 78),
    ("Ian Jackson", "SG", 2, 76, 80),
    ("Bennett Stirtz", "PG", 4, 76, 82),
    ("Karter Knox", "SG", 1, 76, 80),
    ("Darrion Williams", "SF", 3, 78, 80),
    ("Silas Demary Jr.", "SG", 3, 74, 80),
    ("Elliot Cadeau", "PG", 3, 75, 82),
    ("Baba Miller", "PF", 3, 73, 80),
    ("Coleman Hawkins", "PF", 4, 72, 78),
    ("Miles Byrd", "SG", 4, 72, 82),
    ("Rob Wright III", "PG", 3, 73, 80),
    ("Chucky Hepburn", "PG", 4, 74, 85),
    ("Baye Fall", "C", 3, 74, 78),
    ("Rueben Chinyelu", "C", 3, 74, 82),
    ("Solo Ball", "SG", 3, 74, 82),
    ("Aday Mara", "C", 2, 74, 78),
    ("Trey Green", "PG", 1, 75, 80),
    ("Jayden Quaintance", "PF", 2, 78, 68),
    # ---- Depth / role players (55-78) ----
    ("Zuby Ejiofor", "PF", 3, 78, 82),
    ("Aaron Bradshaw", "C", 2, 74, 78),
    ("Cliff Omoruyi", "C", 4, 72, 82),
    ("Hunter Sallis", "SG", 4, 74, 82),
    ("Terrance Arceneaux", "SF", 3, 70, 80),
    ("Xaivian Lee", "PG", 3, 76, 82),
    ("Ta'Lon Cooper", "PG", 4, 70, 82),
    ("Chris Youngblood", "SG", 4, 72, 82),
    ("Boo Buie", "PG", 4, 74, 78),
    ("Cam Spencer", "SG", 4, 72, 80),
    ("Trey Kaufman-Renn", "PF", 4, 78, 85),
    ("Fletcher Loyer", "PG", 4, 74, 85),
    ("Sam Waardenburg", "PF", 4, 66, 82),
    ("Riley Kugel", "SG", 3, 72, 80),
    ("Alex Karaban", "PF", 4, 76, 85),
    ("Tristan Newton", "PG", 4, 72, 82),
    ("Hunter Dickinson", "C", 4, 80, 85),
    ("Kyle Filipowski", "C", 4, 78, 80),
    ("Terrence Shannon Jr.", "SG", 4, 78, 78),
    ("Jamal Shead", "PG", 4, 74, 85),
    ("Tyson Walker", "PG", 4, 72, 82),
    ("Judah Mintz", "PG", 3, 70, 80),
    ("Wooga Poplar", "SG", 4, 68, 82),
    ("Reece Beekman", "PG", 4, 76, 85),
    ("Ryan Nembhard", "PG", 4, 74, 82),
    ("Jaxson Robinson", "SG", 4, 74, 82),
    ("Malik Reneau", "PF", 3, 74, 80),
    ("Trey Townsend", "PF", 4, 68, 82),
    ("Steven Ashworth", "PG", 4, 68, 82),
    ("Isaiah Stevens", "PG", 4, 68, 82),
    ("Amari Williams", "C", 4, 72, 85),
    ("RJ Davis", "PG", 4, 78, 82),
    ("Devin Carter", "SG", 4, 78, 75),
    ("Kevin McCullar Jr.", "SF", 4, 74, 78),
    ("Tyler Kolek", "PG", 4, 74, 78),
    ("Isaac McKneely", "SG", 3, 70, 82),
    ("Zvonimir Ivisic", "C", 3, 70, 78),
    ("Assane Diop", "C", 2, 66, 78),
    ("Trentyn Flowers", "SF", 2, 70, 80),
    ("Chance Westry", "SG", 2, 66, 78),
    ("Bryce Hopkins", "PF", 4, 72, 80),
    ("Nique Clifford", "SG", 4, 76, 82),
    ("PJ Hall", "C", 4, 70, 82),
    ("Jaxson Kohler", "C", 2, 62, 78),
    ("Malachi Moreno", "C", 1, 68, 80),
    ("Darius Acuff Jr.", "PG", 1, 70, 80),
    ("Braylon Mullins", "SG", 1, 68, 82),
    ("Jerry Easter II", "SG", 1, 68, 80),
    ("Igor Milicic Jr.", "PF", 3, 68, 80),
]
...
CFB_RAW: list[tuple[str, str, int, int, int]] = [
    # ---- Elite (88-97) ----
    ("Jeremiah Smith", "WR", 2, 97, 88),
    ("Arch Manning", "QB", 2, 90, 82),
    ("Ryan Williams", "WR", 2, 92, 82),
    ("Jeremiyah Love", "RB", 3, 90, 82),
    ("Rueben Bain Jr.", "DE", 3, 90, 82),
    ("Caleb Downs", "S", 3, 93, 88),
    ("Francis Mauigoa", "OT", 3, 88, 88),
    ("Peter Woods", "DT", 3, 88, 85),
    ("LaNorris Sellers", "QB", 2, 87, 85),
    ("Fernando Mendoza", "QB", 3, 86, 85),
    # ---- Quality (74-87) ----
    ("Kadyn Proctor", "OT", 3, 87, 85),
    ("Julian Sayin", "QB", 2, 82, 82),
    ("Garrett Nussmeier", "QB", 4, 80, 80),
    ("DJ Lagway", "QB", 2, 85, 75),
    ("Drew Allar", "QB", 4, 82, 82),
    ("Cade Klubnik", "QB", 4, 80, 85),
    ("Dante Moore", "QB", 2, 78, 82),
    ("Dylan Raiola", "QB", 2, 84, 82),
    ("Sam Leavitt", "QB", 3, 82, 85),
    ("Avery Johnson", "QB", 3, 78, 85),
    ("Diego Pavia", "QB", 4, 78, 88),
    ("Rocco Becht", "QB", 3, 79, 84),
    ("Haynes King", "QB", 4, 76, 82),
    ("Behren Morton", "QB", 4, 77, 78),
    ("Jonah Coleman", "RB", 3, 82, 85),
    ("Jadarian Price", "RB", 3, 80, 82),
    ("Nicholas Singleton", "RB", 4, 83, 85),
    ("Kaytron Allen", "RB", 4, 82, 85),
    ("Jam Miller", "RB", 3, 76, 82),
    ("CJ Baxter", "RB", 3, 78, 78),
    ("Emmett Johnson", "RB", 3, 78, 85),
    ("Le'Veon Moss", "RB", 3, 77, 78),
    ("Makhi Hughes", "RB", 3, 78, 82),
    ("Antonio Williams", "WR", 3, 84, 85),
    ("Makai Lemon", "WR", 3, 84, 85),
    ("Denzel Boston", "WR", 3, 82, 85),
    ("Nyck Harbor", "WR", 2, 80, 82),
    ("Elijah Sarratt", "WR", 4, 79, 85),
    ("Duce Robinson", "WR", 3, 80, 84),
    ("Kevin Concepcion", "WR", 2, 80, 82),
    ("Zachariah Branch", "WR", 3, 80, 82),
    ("Isaiah Bond", "WR", 3, 76, 78),
    ("Evan Stewart", "WR", 3, 78, 75),
    ("Nic Anderson", "WR", 3, 76, 75),
    ("Chris Bell", "WR", 3, 78, 82),
    ("Max Klare", "TE", 3, 80, 85),
    ("Jack Endries", "TE", 4, 78, 85),
    ("Tyler Booker", "OG", 4, 85, 88),
    ("Josh Simmons", "OT", 4, 84, 70),
    ("David Bailey", "DE", 4, 84, 85),
    ("Anto Saka", "DE", 3, 82, 85),
    ("Colin Simmons", "DE", 2, 82, 78),
    ("T.J. Parker", "DE", 2, 80, 82),
    ("Sonny Styles", "LB", 3, 84, 85),
    ("Danny Stutsman", "LB", 4, 84, 85),
    ("Whit Weeks", "LB", 3, 82, 88),
    ("Arvell Reese", "LB", 3, 82, 82),
    ("Deontae Lawson", "LB", 4, 80, 78),
    ("Malik Muhammad", "CB", 3, 79, 85),
    ("Xavier Lucas", "CB", 3, 80, 85),
    ("A.J. Harris", "CB", 3, 78, 82),
    ("Cameron Calhoun", "CB", 3, 78, 85),
    ("Domani Jackson", "CB", 4, 78, 78),
    ("Peyton Bowen", "S", 3, 78, 85),
    ("Luke Kromenhoek", "QB", 2, 74, 82),
    ("Cutter Boley", "QB", 2, 73, 80),
    # ---- Depth / role players (55-78) ----
    ("Marcel Reed", "QB", 2, 76, 82),
    ("Kurtis Rourke", "QB", 4, 74, 75),
    ("Blake Horvath", "QB", 3, 74, 85),
    ("Taylen Green", "QB", 3, 76, 85),
    ("Kaidon Salter", "QB", 3, 74, 82),
    ("Noah Fifita", "QB", 3, 76, 82),
    ("Demond Williams Jr.", "QB", 2, 74, 85),
    ("Chandler Morris", "QB", 4, 70, 80),
    ("Devin Brown", "QB", 3, 68, 80),
    ("Jalon Daniels", "QB", 4, 72, 68),
    ...
    ("Josh Cameron", "WR", 4, 66, 82),
    ("Gunnar Helm", "TE", 4, 74, 82),
    ("Oscar Delp", "TE", 3, 70, 82),
    ("Walker Lyons", "TE", 3, 68, 82),
    ("Miles Kitselman", "TE", 3, 66, 82),
    ("Spencer Fano", "OT", 3, 84, 85),
    ("Trey Zuhn II", "OT", 4, 76, 82),
    ("Emery Jones Jr.", "OT", 4, 74, 82),
    ("Isaiah World", "OT", 3, 74, 82),
    ("Caleb Lomu", "OT", 2, 70, 82),
    ("Deone Walker", "DT", 3, 78, 82),
    ("Christen Miller", "DT", 3, 74, 80),
